...
# def generate_numbered_html_section(
#     start_number: int, field_data_heading: str, field_data: list, line_length: int = 100
# ) -> tuple:
#     """
#     Generates an HTML section with a heading and text split into numbered lines,
#     applying right-justified alignment for each line.
    
#     Args:
#         start_number (int): The starting line number.
#         field_data_heading (str): The heading for this section.
#         field_data (list): A list of strings (paragraphs) to be split and numbered.
#         line_length (int): Approximate maximum number of characters per line.

#     Returns:
#         tuple: A tuple containing:
#             - str: The HTML section with line numbers.
#             - int: The updated line count.
#     """
#     # Heading with center alignment for specific sections
#     heading = f"<p><b><span>{field_data_heading}</span></b></p>"
#     if field_data_heading in {"ABSTRACT", "CLAIMS"}:
#         heading = f'<p style="text-align:center;"><b><span>{field_data_heading}</span></b></p>'

#     # Page break for sections with certain headings
#     if field_data_heading in {"FIGURES", "CLAIMS", "ABSTRACT"}:
#         page_break = '<div style="page-break-before: always;"></div>'
#         heading = page_break + heading
...
#             else:
#                 # Format the line to be justified by adding spaces between words
#                 if current_line:
#                     if len(current_line) > 1:
#                         extra_spaces = line_length - sum(len(w) for w in current_line)
#                         spaces_needed = len(current_line) - 1
#                         space_between_words = extra_spaces // spaces_needed
#                         extra_space_mod = extra_spaces % spaces_needed

#                         # Distribute spaces between words
#                         justified_line = ""
#                         for i, w in enumerate(current_line[:-1]):
#                             justified_line += w + " " * (space_between_words + (1 if i < extra_space_mod else 0))
#                         justified_line += current_line[-1]  # add the last word without extra space
#                     else:
#                         justified_line = current_line[0]  # single-word line

#                     # Add the justified line with line numbering
#                     numbered_line = f'<p style="text-align: justify; margin-right: 10px;"><span style="margin-right: 10px;">{line_count}.</span>{justified_line}</p>'
...
#         if current_line:
#             numbered_line = f'<p style="text-align: justify; margin-right: 10px;"><span style="margin-right: 10px;">{line_count}.</span>{" ".join(current_line)}</p>'
#             numbered_lines.append(numbered_line)
#             line_count += 1

#     html_section = heading + "".join(numbered_lines)
#     updated_start_number = line_count
...
def generate_numbered_html_section(
    ...
    for paragraph in field_data:
        ...
        # Add any remaining words in the current line
        if current_line:
            justified_line = " ".join(current_line).ljust(line_length)  # Adjust for final line to align right if needed
            numbered_line = f'<p style="text-align: justify; margin-right: 10px;"><span style="margin-right: 10px;">{line_count}.</span>{justified_line}</p>'
            numbered_lines.append(numbered_line)
            line_count += 1
...
# this is the function use for json data to converitng html content 
...
def save_pdf_from_html(html_content, output_filename="output1.pdf"):
